...
version = str(ast.literal_eval(_version_re.search(
    read_file(os.path.join(_source_dir, 'mlstorage_server/__init__.py'))).group(1)))
...
setup(
    name='MLStorage Server',
    version=version,
    url='https://github.com/haowen-xu/mlstorage-server/',
    license='MIT',
    author='Haowen Xu',
    author_email='haowen.xu@outlook.com',
    description='The server side for MLStorage.',
    long_description=__doc__,
    packages=find_packages(
        '.', include=['mlstorage_server', 'mlstorage_server.*']),
    package_data={
        'mlstorage_server': [
            'assets/*',
            'assets/static/css/*',
